...
## Verification
Поведение объекта почти всегда реализуется в `sim` и проверяется детерминированным
юнит-тестом там же (см. `src/sim/AGENTS.md`). `engine` тестами не покрывается —
проверяется запуском (`vite`) и глазами. Критерий готовности объекта: всё его
поведение зелёное в `sim`-тестах, игра визуально работает.

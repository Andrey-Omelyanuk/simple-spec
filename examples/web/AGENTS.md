...
## Verification
Как проверяется, что история реализована на этом уровне.

- **Unit (Jest):** логика компонентов, хуки, утилиты. Файл рядом с кодом:
  `Component.test.tsx`.
- **E2E (Storybook):** каждый сценарий истории = отдельная story в
  `*.stories.tsx`, прогон через `@storybook/test-runner`.
- **Связь с историей:** в начале теста и story — ссылка на историю:
  `// story: <unique-name>`.
- **Критерий готовности:** для истории есть unit-покрытие логики и хотя бы одна
  e2e-story на каждый сценарий; всё зелёное.

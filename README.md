...
Команды и подложку в существующий проект ставит `install.sh`:

```
./install.sh <путь-к-проекту> [служебная-папка]
```

Служебная папка по умолчанию `.opencode` (для Claude Code укажи `.claude`). Скрипт
подстраивает подпапку команд под инструмент, кладёт рядом подложку (`OBJECT.md`,
`AGENTS.md`, скрипт проверки) и создаёт `oos/` и `stories/`, если их нет. Повторный
запуск обновляет кит; объекты и истории не трогает.

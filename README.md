...
(минимум: unit и e2e). Как тест связывается с объектом и почему имена уникальны —
`src/OBJECT.md`.
...
`.claude`). Скрипт подстраивает подпапку команд под инструмент, кладёт рядом
подложку (`OBJECT.md`, `AGENTS.md`, скрипт проверки) и создаёт `oos/` и `stories/`,
если их нет.

**Глобально:** команды едут в командную папку инструмента (opencode →
`~/.config/opencode`, claude → `~/.claude`), подложка — в `oos-kit/` рядом, ссылки
в командах переписываются на абсолютный путь к ней. `oos/` и `stories/` не
создаются — они пер-проектные, их заводят сами команды в текущем проекте.

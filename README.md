...
состояния, как себя ведёт, с кем связан. Бывает **сущностью** (суть в поведениях:
Сердечко, Портал) или **процессом** (суть в переходах состояний: Заявка, Заказ).
Взаимодействие принадлежит **активному** объекту. Правила и шаблон — `src/OBJECT.md`.

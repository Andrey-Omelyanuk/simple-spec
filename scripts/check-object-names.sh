#!/usr/bin/env bash
# Проверяет правило из OBJECT.md («Связь с кодом»): имя файла объекта уникально
# внутри каждой папки oos/ (включая все вложенные). Это позволяет переносить
# объекты между папками, не переделывая ссылки `// object:`.
#

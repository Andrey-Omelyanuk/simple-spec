...
`src/en/templates/<name>/` with a set of `AGENTS.md` skeletons per level. `/architect`
reads the template folder, expands the levels for the domain and creates files
after consent.

The level format is one for all, in `src/en/LEVEL.md`; a template only defines the
...
`install.sh` are needed: it copies `src/en/templates` as a whole.